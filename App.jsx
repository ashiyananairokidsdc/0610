...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubP = onSnapshot(
      ...
      (err) => { console.error(err); setLoadedP(true); }
    );
    const unsubM = onSnapshot(
      ...
      (err) => { console.error(err); setLoadedM(true); }
    );
    return () => { unsubP(); unsubM(); };
  }, []);

  useEffect(() => { if (loadedP && loadedM) setLoading(false); }, [loadedP, loadedM]);
  ...
  const totalValue = useMemo(
    () => products.reduce((a, p) => a + unitPrice(p) * totalStock(p), 0),
    [products]
  );
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="leading-tight">
              ...
              <div className="text-[11px] text-slate-400">歯科医院</div>
            </div>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-5">
        <div className="grid grid-cols-3 gap-3 mb-5">
          <Stat label="登録商品数" value={products.length} unit="点" icon={<Package className="w-4 h-4" />} />
          <button onClick={() => setCatFilter("reorder")} className="text-left">
            <Stat label="要発注" value={reorderList.length} unit="点" alert={reorderList.length > 0} icon={<AlertTriangle className="w-4 h-4" />} />
          </button>
          <Stat label="在庫総額（概算）" value={yen(totalValue)} icon={<Tag className="w-4 h-4" />} />
        </div>

        {reorderList.length > 0 && catFilter !== "reorder" && (
          <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-3">
            <div className="flex items-center gap-2 text-amber-800 font-medium text-sm mb-2">
              <AlertTriangle className="w-4 h-4" /> 発注ライン以下の商品が {reorderList.length} 点あります
            </div>
    ...
    <button onClick={onEdit}
      ...
      <div className="flex gap-3 p-3">
        <div className="w-16 h-16 rounded-lg bg-slate-100 shrink-0 overflow-hidden flex items-center justify-center">
          {p.image ? <img src={p.image} alt="" className="w-full h-full object-cover" /> : <ImageIcon className="w-6 h-6 text-slate-300" />}
        </div>
